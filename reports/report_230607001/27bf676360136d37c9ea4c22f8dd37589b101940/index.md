...
**上述肿瘤信号强度的最终值是我们通过高达几十亿（~2-3G）碱基对的测序数据所测得的全转录组基因表达值，加上我们专有的 AI 模型计算而来。全转录组基因表达值测定总图表如下：**
...
上述肿瘤信号强度的最终值是我们通过高达几十亿（~2-3G）碱基对的测序量加上我们专有的 AI 模型计算而来，包含了如下图所示的**4个主要模块的数十道工序**。各个中间步骤的具体结果文件可以从 [附录：全部文件下载链接](#files) 中获取。
...
> 检测和计算结果工序流程图（基于高达几十亿（~2-3G）碱基对的测序数据和独有AI模型）：
